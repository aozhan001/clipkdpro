...
from .offline_teacher_cache import (
    OfflineTeacherCacheDatasetWrapper,
    SampleIDDatasetWrapper,
    build_csv_dataset_sample_ids,
    build_csv_dataset_source_prefix,
    build_sequential_sample_ids,
)
from .offline_sampler import build_offline_sample_filter_sampler
...
class ImageNetCsvDataset(Dataset):
    def __init__(
        ...
        sep="\t",
        tokenizer=None,
        deterministic_caption_templates=False,
    ):
        logging.debug(f'Loading csv data from {input_filename}.')
        df = pd.read_csv(input_filename, sep=sep)
        ...
        self.root = data_root
        self.tokenize = tokenizer
        self.deterministic_caption_templates = deterministic_caption_templates

    def render_caption(self, idx):
        definition = "The definition of %s is %s" % (self.class_name[idx], self.captions[idx])
        if self.deterministic_caption_templates:
            template_index = idx % self.imagenet_templates_num
        else:
            template_index = np.random.choice(self.imagenet_templates_num)
        template_caption = self.imagenet_templates[template_index](self.class_name[idx])
        return template_caption + " " + definition
    ...
    def __getitem__(self, idx):
        ...
        caption = self.render_caption(idx)
...
def is_retrieval_dataset_path(data_path):
    if not data_path or not isinstance(data_path, str):
        return False
    if os.path.isfile(data_path) or not os.path.isdir(data_path):
        return False
    return (
        os.path.exists(os.path.join(data_path, "test_captions.pt")) and
        os.path.exists(os.path.join(data_path, "test_img_keys.tsv"))
    )


def maybe_wrap_with_sample_ids(dataset, dataset_label=None, input_filename=None, dataset_idx=0):
    sample_ids = None
    if input_filename is not None and isinstance(dataset, (CsvDataset, ImageNetCsvDataset)):
        source_prefix = build_csv_dataset_source_prefix(input_filename, dataset_idx)
        sample_ids = build_csv_dataset_sample_ids(dataset, source_prefix)
    elif dataset_label is not None:
        sample_ids = build_sequential_sample_ids(dataset_label, len(dataset))

    if sample_ids is None:
        return dataset
    return SampleIDDatasetWrapper(dataset, sample_ids)


def maybe_wrap_with_offline_teacher_cache(dataset, args, is_train=False, dataset_label=None, input_filename=None, dataset_idx=0):
    if not is_train:
        return dataset
    if not getattr(args, "use_offline_teacher_cache", False):
        return dataset
    teacher_cache_path = getattr(args, "teacher_cache_path", None)
    if not teacher_cache_path:
        raise ValueError("--use-offline-teacher-cache requires --teacher-cache-path to be set.")
    if isinstance(dataset, RetrievalDataset):
        raise ValueError("Offline teacher cache is not supported for RetrievalDataset because sample alignment is ambiguous.")
    if isinstance(dataset, datasets.ImageFolder):
        raise ValueError("Offline teacher cache is not supported for ImageFolder-only datasets because no paired text is available.")
    dataset = maybe_wrap_with_sample_ids(
        dataset,
        dataset_label=dataset_label,
        input_filename=input_filename,
        dataset_idx=dataset_idx,
    )
    return OfflineTeacherCacheDatasetWrapper(dataset, teacher_cache_path)
...
def get_wds_dataset(args, preprocess_img, is_train, epoch=0, floor=False, tokenizer=None):
    if is_train and getattr(args, "use_offline_teacher_cache", False):
        raise ValueError("Offline teacher cache is not supported for dataset_type='webdataset' in the current implementation.")
...
def get_icar(args, preprocess_fn, is_train, epoch=0, tokenizer=None):
    if getattr(args, "use_offline_teacher_cache", False):
        raise ValueError("Offline teacher cache is not supported for dataset_type='icar' in the current implementation.")
    ...
    sup_dataset = maybe_wrap_with_offline_teacher_cache(
        sup_dataset,
        args,
        is_train=is_train,
        input_filename=train_dataset_names[0],
        dataset_idx=0,
    )
    vl_dataset = maybe_wrap_with_offline_teacher_cache(
        vl_dataset,
        args,
        is_train=is_train,
        input_filename=train_dataset_names[1],
        dataset_idx=1,
    )
    ...
    sup_dataloader = DataLoader(
        ...
        sampler=sup_sampler,
        drop_last=is_train,
        collate_fn=getattr(sup_dataset, "collate_fn", None),
    )
    ...
    vl_dataloader = DataLoader(
        ...
        sampler=vl_sampler,
        drop_last=is_train,
        collate_fn=getattr(vl_dataset, "collate_fn", None),
    )
...
def get_vl_imagenet(args, preprocess_fn, is_train, epoch=0, tokenizer=None):
    if getattr(args, "use_offline_teacher_cache", False):
        raise ValueError("Offline teacher cache is not supported for dataset_type='vl_imagenet' in the current implementation.")
...
def get_csv_dataset(args, preprocess_fn, is_train, epoch=0, tokenizer=None):
    ...
    if 'imagenet' in input_filename:
        dataset = ImageNetCsvDataset(
            ...
            sep=args.csv_separator,
            tokenizer=tokenizer,
            deterministic_caption_templates=getattr(args, "use_offline_teacher_cache", False) and is_train)
    elif is_retrieval_dataset_path(input_filename):
        ...
        dataset = CsvDataset(
            ...
            sep=args.csv_separator,
            tokenizer=tokenizer)
    dataset = maybe_wrap_with_offline_teacher_cache(
        dataset,
        args,
        is_train=is_train,
        input_filename=input_filename,
    ...
    sampler = build_offline_sample_filter_sampler(dataset, args, is_train=is_train)
    if sampler is None:
...
def get_csv_multi_dataset(args, preprocess_fn, is_train, epoch=0, tokenizer=None):
    ...
    if is_train and getattr(args, "use_offline_teacher_cache", False):
        dataset = OfflineTeacherCacheDatasetWrapper(dataset, args.teacher_cache_path)
    sampler = build_offline_sample_filter_sampler(dataset, args, is_train=is_train)
    if sampler is None:
        sampler = DistributedSampler(dataset) if args.distributed and is_train else None
    num_samples = len(sampler) if sampler is not None else len(dataset)
