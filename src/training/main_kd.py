...
from open_clip import create_kd_model_and_transforms, create_model_and_transforms, trace_model, get_model_config, get_tokenizer
from training.data import get_data
from training.distributed import is_master, init_distributed_device, world_info_from_env
from training.logger import setup_logging
from training.offline_kd import is_offline_only_kd_enabled
...
def get_model_embed_dim(model_name):
    ...
    if model_cfg is None:
        raise RuntimeError(f"Model config for {model_name} was not found.")
    return int(model_cfg["embed_dim"])
...
def main(args):
    ...
    args.t_model = args.t_model.replace('/', '-')
    args.t_embed_dim = get_model_embed_dim(args.t_model)
    args.s_embed_dim = get_model_embed_dim(args.model)
    ...
    if t_model is not None:
        ...
        logging.info("Offline-only KD mode enabled. Skipping teacher checkpoint loading and online teacher forward.")
        if any(
            value > 0.0 for value in (
                args.alpha_ckd_loss,
                args.alpha_icl_loss,
                args.alpha_cross_kd_loss,
                args.alpha_fd_loss,
                args.alpha_gd_loss,
                args.alpha_afd_loss,
            )
            ...
            logging.warning(
                "Offline-only KD mode ignores online teacher loss weights "
                "(alpha_ckd_loss, alpha_icl_loss, alpha_cross_kd_loss, alpha_fd_loss, alpha_gd_loss, alpha_afd_loss)."
            )
    ...
    loss = KDClipLoss(
        ...
        use_horovod=args.horovod).to(device)
    ...
    if args.t_eval and t_model is not None:
        print('evaluate teacher:')
        evaluate(t_model, data, start_epoch, args, writer)
    elif args.t_eval:
        logging.warning("Teacher evaluation is unavailable in offline-only KD mode because no teacher model was loaded.")
